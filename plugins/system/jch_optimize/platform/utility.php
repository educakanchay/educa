...
class JchPlatformUtility implements JchInterfaceUtility
{
        ...
        public static function log($message, $category)
        {
                ...
                JLog::addLogger(
                        array(
                        'text_file' => 'plg_jch_optimize.errors.php'), JLog::ALL, $category
                );
                JLog::add(JText::_($message), JLog::WARNING, $category);
        }
        ...
        public static function RFC1123DateAdd($filemtime, $days)
        {
                ...
                $iTimeMFile = JFactory::getDate($filemtime);

                $aTime['filemtime'] = $iTimeMFile->format('D, d M Y H:i:s');

                $iTimeMFile->add(DateInterval::createFromDateString($days . ' days'));
                $aTime['expiry'] = $iTimeMFile->format('D, d M Y H:i:s');
